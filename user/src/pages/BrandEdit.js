...
export const BrandEdit = ({ access_token, loading, client }) => {
  const history = useNavigate();
  const { id } = useParams();

  const getBrand = async (id) => {
    const response = await client.get("/brands/getOne/" + id);
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(values);
    const data = values;
    const response = await client.put("/brands/update/" + id, data, {
      headers: {
